...
class MinimumStepSizeExceededError(NumericalIntegrationError):
    """ Exception raised when an attempted integration step exceeds a minimum step size threshold.
    ...
    def __init__(self, h_sec: float, h_min_sec: float):
        self.message = f"Minimum integration step size exceeded. A {h_sec} second step was attempted with a minimum " \
                       f"integration step size of {h_min_sec} seconds."
        self.h = h_sec
