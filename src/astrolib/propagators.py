...
class Ephemeris(PropagatorBase):
    ...
    def duration(self) -> TimeSpan:
        return (
            ...
            if (self.start_epoch.is_defined() and self.end_epoch.is_defined())
            else TimeSpan.zero()
        )
...
class Integrator(PropagatorBase):
    ...
    # pylint: disable=arguments-differ
    def get_state(
        self, epoch: TimeSpan, X_0: StateVector, *args, **kwargs
    ) -> StateVector:
        raise NotImplementedError

    # pylint: disable=arguments-differ
    def get_states(
        self, epochs: List[TimeSpan], X_0: StateVector, *args, **kwargs
    ) -> Ephemeris:
    ...
    def _evaluate_dynamics(
        self, epoch: TimeSpan, state: Matrix, state_vector_type: Type
    ) -> Matrix:
        return self.dynamics_model.evaluate(
            state_vector_type.from_column_matrix(epoch, state)
        )
...
class FixedStepIntegrator(Integrator):
    ...
    def __init__(
        ...
        integration_func: Callable[
            [TimeSpan, Matrix, TimeSpan, Callable[[TimeSpan, Matrix], Matrix]],
    ...
    def get_state(self, epoch: TimeSpan, X_0: StateVector) -> StateVector:
        ...
        while t_n != epoch:
            t_n, x_n, _ = self._integration_func(
                t_n,
                x_n,
                self.step_size,
    ...
    def _check_time_step_validity(self, t_0: TimeSpan, t_f: TimeSpan) -> bool:
        if not ((t_f - t_0).to_seconds() % self.step_size.to_seconds()) == 0.0:
            raise ValueError(
                "Unable to propagate to desired epoch using fixed-step "
