...
def interpolate(x_vals: Matrix, y_vals: Matrix, x_0: float) -> float:
    ...
    if (
        not (x_vals.is_row_matrix() or y_vals.is_row_matrix())
        or x_vals.size != y_vals.size
    ):
        raise ValueError(
            "Invalid input data provided to Lagrange polynomial fit function. X- and Y-data must be row or column vectors and the same length."
        )
    y_0 = 0.0
